...
<img src="https://voyager0587.oss-cn-guangzhou.aliyuncs.com/%E7%AC%94%E8%AE%B0%E5%9B%BE%E7%89%87/202307071224041.png" alt="image-20230707122403945" style="zoom:67%;" />
...
接下来创建标题Tab的适配器`ItemFragmentAdapter`，代码如下：
...
最后不要忘记在`AndroidManifest.xml`中添加网络权限
...
##### ③在`activity_main.xml`中添加`Banner`控件
...
public class MainActivity extends AppCompatActivity {
    ...
    private void initData(){
        banner.setAdapter(new BannerImageAdapter<DataBean>(DataBean.getData()) {
            @Override
            public void onBindView(BannerImageHolder holder, DataBean data, int position, int size) {
                Glide.with(holder.imageView)
                        .load(data.getImageUrl())
                        .thumbnail(Glide.with(holder.imageView).load(R.drawable.ic_launcher_background))
                ...
                .setOnBannerListener((data, position) -> {
                    Snackbar.make(banner,"你点击了第"+(position+1)+"个轮播图",Snackbar.LENGTH_SHORT).show();
...
<img src="https://voyager0587.oss-cn-guangzhou.aliyuncs.com/%E7%AC%94%E8%AE%B0%E5%9B%BE%E7%89%87/202306172153562.gif" alt="device-2023-06-17-21_-original-original" style="zoom:25%;" />



##### ⑥实现带标题的轮播图

> 实现带有标题的轮播图要自己设计`Banner`的适配器

###### 1.首先先创建item的布局

<span style="background-color:Darkorange ;"><b>注意：</b></span>item布局的长宽必须全都是match_parent!
...
###### 2.创建`Banner`的适配器`BannerAdaptor`

```java
public class BannerAdapter extends com.youth.banner.adapter.BannerAdapter<DataBean, BannerAdapter.ViewHolder> {

    public BannerAdapter(List<DataBean> dataList) {
        super(dataList);
    }

    @Override
    public ViewHolder onCreateHolder(ViewGroup parent, int viewType) {
        final ViewHolder holder=new ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.banner_item_list,parent,false));
        return holder;
    }
...
###### 3.修改`MainActiity`中的代码

```java
private void initData(){
   banner.setAdapter(new BannerAdapter(DataBean.getData())).addBannerLifecycleObserver(this)
...
<img src="https://voyager0587.oss-cn-guangzhou.aliyuncs.com/%E7%AC%94%E8%AE%B0%E5%9B%BE%E7%89%87/202306172318904.gif" alt="device-2023-06-17-23_-original-original" style="zoom:25%;" />
